...
class TacticalPDF(FPDF):
    ...
    def draw_footer_elements(self):
        # Rodapé Técnico com Checklist e Ocorrência
        self.set_y(-72)
...
if st.session_state.alvos and st.button("🛰️ GERAR DOSSIÊ TÁTICO FINAL", key="pdf"):
    ...
    for i, alvo in enumerate(st.session_state.alvos):
        ...
        # 1. Cabeçalho da Operação (Só aparece no topo da folha se for o início do conteúdo)
        # O modelo exige o nome da Op e Comando em destaque
        ...
        if alvo['foto_alvo']:
            img_a = Image.open(alvo['foto_alvo']); img_a.save(f"tmp_a_{i}.png")
        ...
        if alvo['foto_casa']:
            img_c = Image.open(alvo['foto_casa']); img_c.save(f"tmp_c_{i}.png")
            pdf.image(f"tmp_c_{i}.png", x=107, y=y_f, w=93, h=50)
            pdf.set_xy(107, y_f + 46); pdf.set_text_color(255,255,255)
            pdf.cell(93, 4, " PERÍMETRO DE ENTRADA", 0, 1, 'L', True)
        
        # 4. Inteligência e Logística com mais espaço
        pdf.set_y(y_f + 58); c_y = pdf.get_y()
        ...
        pdf.set_x(10); pdf.cell(95, 4, 'ENDEREÇO DE CUMPRIMENTO', 0, 0)
        ...
        # 5. Seção GPS deslocada para baixo (limite de 2 endereços por página)
        pdf.set_y(pdf.get_y() + 6)
        for e_idx, ender in enumerate(alvo['enderecos'][:2]):
            if ender.strip():
                pdf.set_fill_color(248, 250, 255); pdf.rect(10, pdf.get_y(), 190, 23, 'F')
                qr_p = gerar_qr(end_origem, ender)
                pdf.image(qr_p, x=12, y=pdf.get_y() + 1.5, w=20, h=20)
                pdf.set_xy(35, pdf.get_y() + 4); pdf.set_font('Helvetica', 'B', 8); pdf.set_text_color(30, 50, 120)
                pdf.cell(0, 4, f'ROTA GPS #{e_idx+1} - {ender.upper()[:45]}', 0, 1)
                pdf.set_font('Helvetica', '', 7); pdf.set_text_color(100, 100, 100)
                pdf.set_x(35); pdf.multi_cell(0, 4, f"ORIGEM: {end_origem}\nDESTINO: {ender}")
                pdf.ln(1); os.remove(qr_p)

        # Rodapé técnico obrigatório em cada dossiê de alvo
        pdf.draw_footer_elements()

    # Saída do Arquivo
    output_name = f"Dossie_SIG_Jaboticabal_{datetime.now().strftime('%d%m%H%M')}.pdf"
