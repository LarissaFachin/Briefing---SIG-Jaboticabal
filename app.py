...
# Estilização Profissional Dark Mode
st.markdown("""
    <style>
...
with st.container(border=True):
    ...
    # PRÉ-PREENCHIMENTO CONFORME SOLICITADO
...
# --- GERAÇÃO DO PDF ---
if st.session_state.alvos and st.button("🛰️ GERAR DOSSIÊ TÁTICO"):
    pdf = TacticalPDF()
    for i, alvo in enumerate(st.session_state.alvos):
        pdf.add_page()
        
        # Dados da Operação: Apenas na Página 1
        if i == 0:
            pdf.set_font('Helvetica', 'B', 18)
            pdf.set_text_color(0, 30, 80)
            pdf.cell(0, 10, nome_op.upper(), 0, 1)
            pdf.set_font('Helvetica', 'B', 10)
            pdf.cell(0, 5, f"UNIDADE DE COMANDO: {unidade.upper()}", 0, 1)
            pdf.ln(2)
        ...
        # Barra do Alvo (Mandado com largura 65 para não cortar)
        ...
        # --- SEÇÃO 1: LOGÍSTICA (VIATURA E EQUIPE) ---
        pdf.set_font('Helvetica', 'B', 10)
        pdf.cell(0, 8, "LOGÍSTICA E EQUIPE OPERACIONAL", 'B', 1)
        pdf.ln(2)
        
        pdf.set_font('Helvetica', 'B', 9)
        pdf.cell(22, 6, "VIATURA: ", 0, 0)
        pdf.set_font('Helvetica', '', 9)
        pdf.cell(0, 6, alvo['viatura'].upper() if alvo['viatura'] else "NÃO INFORMADA", 0, 1)
        
        pdf.set_font('Helvetica', 'B', 9)
        pdf.cell(22, 6, "EFETIVO: ", 0, 0)
        pdf.set_font('Helvetica', '', 9)
        agentes_str = " | ".join([a for a in alvo['agentes'] if a.strip()])
        pdf.multi_cell(0, 6, agentes_str if agentes_str else "NÃO INFORMADO")
        pdf.ln(2)

        # --- SEÇÃO 2: ROTAS (ENDEREÇOS E GPS) ---
        pdf.set_font('Helvetica', 'B', 10)
        pdf.cell(0, 8, "ROTAS GPS TÁTICAS", 'B', 1)
        pdf.ln(2)
        ...
        for end in alvo['enderecos']:
            if end.strip():
                ...
                pdf.image(qr_path, x=10, y=curr_y, w=22)
                pdf.set_xy(35, curr_y + 4)
                pdf.set_font('Helvetica', 'B', 8)
                pdf.multi_cell(0, 4, f"DESTINO: {end.upper()}\nORIGEM: {end_origem.upper()}")
                pdf.set_y(curr_y + 25)
                os.remove(qr_path)

    output_name = "Briefing_SIG_Jaboticabal.pdf"
    pdf.output(output_name)
    with open(output_name, "rb") as f:
        st.download_button("💾 BAIXAR DOCUMENTO FINAL", f, file_name=output_name)
